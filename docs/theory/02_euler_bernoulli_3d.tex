...
\subsection{Element-level transformation}

The $12\times 12$ element transformation $\bmath{T}$ is block-diagonal with four copies of $\bmath{\lambda}$, one per DOF triplet:
\[
\bmath{T} = \mathrm{diag}\bigl(\bmath{\lambda},\bmath{\lambda},\bmath{\lambda},\bmath{\lambda}\bigr).
\]
The four blocks correspond to (uA-translations, $\theta_A$, uB-translations, $\theta_B$). Both translations and the small-rotation pseudovector are transformed by the same $\bmath{\lambda}$; this is exact for translations and consistent for the linear EB rotation DOFs (whose conventions are fixed in the local frame and propagated to the global frame by $\bmath{\lambda}^\mathsf{T}$).
...
$\bmath{T}$               & \texttt{buildTransformation3D}        & $\mathrm{diag}(\bmath{\lambda},\bmath{\lambda},\bmath{\lambda},\bmath{\lambda})$ \\
\bottomrule
