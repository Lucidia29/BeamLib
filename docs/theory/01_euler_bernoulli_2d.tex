...
\section{Consistent Mass Matrix}
...
\begin{enumerate}
    ...
    \item Scatter $\bmath{r}_g^e$, $\bmath{k}_g^e$ into the free-DOF system using the DofMap. Entries with any fixed DOF on either side are dropped (Batch 2A only supports homogeneous Dirichlet constraints; PROJECT\_SPEC \S 11).
\end{enumerate}
