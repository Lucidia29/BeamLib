...
% implemented so far (Phase 1, Batches 1--3).
%
...
% manual is the unified, self-contained entry point that ties the two
...
\large Phase 1, Batches 1--3 (EB2D + EB3D)}
...
This manual gives the unified theoretical foundation for the beam elements implemented in BeamLib Phase~1 so far: the planar Euler--Bernoulli (EB2D) element from Batches~2A/2B, and the spatial Euler--Bernoulli (EB3D) element from Batch~3. It is intended as a single reference that a reader can read end-to-end to understand the kinematic assumptions, sign conventions, finite-element formulation, coordinate transformations, mass-matrix strategy, and verification framework that all current BeamLib elements share.

\section*{Scope versus per-batch chapters}

Two batch-level theory documents already exist and remain authoritative for the full derivations and the complete formula-to-C++ index tables:
...
This unified manual condenses both, with cross-references back to the per-batch chapters when finer detail is needed. Anything the unified manual states is either re-derived here or cited to the corresponding section in the per-batch chapter.
...
Chapter~\ref{ch:framework} sets the language for both elements: DOF ordering, sign conventions, residual semantics, transformation philosophy, assembly flow. Chapters~\ref{ch:eb2d} and~\ref{ch:eb3d} apply that language to each element with self-contained derivations of the element-specific stiffness and mass matrices and the verification cases. Appendix~\ref{app:tests} catalogs the implemented unit tests.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Future Timoshenko (Batches~4/5)}: will add bending rotary inertia $\rho I$.
...
The full Phase 1 (Batches 1--3) test list and what each one covers. Run with \texttt{ctest --test-dir build --output-on-failure}; all currently pass.
...
\bottomrule
\caption{Phase 1 test catalog (15 tests, all PASS).}
\end{longtable}
